...
## 自動テスト
自動でのテストにはpytestを使用しています。最低限のテストは自動テストで行えますが、完璧ではないため手動でのテストを優先してください。

### 注意
テスト項目の拡充の際には、次のことに注意してください。

- 自動テストでは、**全ての**機能が正しく動作し、RFCなどに記載されている**全ての**仕様に**完全に**準拠していることを検証する必要があります。
- 自動テストに現在のKaedeの問題をテストに反映させないでください。
    - つまり、Kaedeの現在の動作に関係なく「実際の仕様に準拠しているか」を検証するようにしてください。
    - ソースコードの現状と一致するかどうかを確認するようなテスト項目を追加した場合、当たり前ですが「現在のKaedeの動作と一致しているか」をテストすることになります。
    - 「現在のKaedeの動作と一致しているか」を検証してしまうと、Kaedeでそれらの仕様を正しく実装できているかの検証になりません。Kaedeの実装に誤った部分があったとしても自動テストでは一切検出されませんし、仕様に準拠させると自動テストが失敗します。それはテストの意味がなく、ただ単に古い誤った実装との比較に過ぎません。
    - テスト項目の設計/拡充の際には、Kaedeの現状の動作を一切信頼しないでください。実際のRFC文書のみを信頼してください。
